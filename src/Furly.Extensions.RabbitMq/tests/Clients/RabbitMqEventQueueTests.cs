...
namespace Furly.Extensions.RabbitMq.Clients
{
    ...
    public sealed class RabbitMqEventQueueTests : IDisposable
    {
        private readonly RabbitMqEventQueueHarness _harness;

        public RabbitMqEventQueueTests(RabbitMqServerFixture server, ITestOutputHelper output)
        {
            var fix = new Fixture();
            var queue = fix.Create<string>();
            _harness = new RabbitMqEventQueueHarness(server, queue, output);
        }
