...
namespace Furly.Extensions.RabbitMq.Clients
{
    ...
    public sealed class RabbitMqEventClientTests : IDisposable
    {
        private readonly RabbitMqEventClientHarness _harness;

        public RabbitMqEventClientTests(RabbitMqServerFixture server, ITestOutputHelper output)
        {
            _harness = new RabbitMqEventClientHarness(server, output);
        }
