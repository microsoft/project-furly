...
namespace Furly.Extensions.Serializers.Newtonsoft
{
    ...
    {
        public static ISerializer Default => new DefaultJsonSerializer();
        public static ISerializer Newtonsoft => new NewtonsoftJsonSerializer();
...
<body>Don't forget me this weekend!</body>
            ...
            var result = Newtonsoft.FromObject(expected);
            var actual = Default.Parse(Default.SerializeToString(expected));
        ...
        {
            var expected = Newtonsoft.FromArray((byte)1, (byte)2, (byte)3);
            var o1 = expected.ConvertTo<byte[]>();
            var json = Default.SerializeToString(o1);
            var actual = Default.Parse(json);
        ...
        {
            var expected = Newtonsoft.FromObject(o).ConvertTo(type);
            var actual = Default.FromObject(o).ConvertTo(type);
        ...
        {
            var expected = Newtonsoft.FromObject(v);
            var actual = Default.FromObject(v);
        ...
        {
            var expected = Newtonsoft.FromObject(new
            {
                Test = "Text",
                Locale = "de"
            });
            var actual = Default.FromObject(new
            {
                Test = "Text",
                Locale = "de"
            });
        ...
        public void SerializerFromObjectContainerToContainerWithObject()
        {
            ...
            {
                Value = Newtonsoft.FromObject(new
                {
                    Test = "Text",
                    Locale = "de"
                })
            };
            var tmp = Default.FromObject(expected);
        ...
        public void SerializerFromObjectContainerToContainerWithArray(object o, Type type)
        {
            ...
            var expected = new TestContainer
            {
                Value = Newtonsoft.FromArray(o, o, o)
            };
            var tmp = Default.FromObject(expected);
            var actual = tmp.ConvertTo<TestContainer>();
            Assert.Equal(expected, actual);
