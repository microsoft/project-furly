...
namespace Furly.Tunnel.AspNetCore.Tests.Services
{
    ...
    [UnitTest]
    public class HttpTunnelMethodClientTests : IClassFixture<InMemoryServerFixture>
    {
        private readonly InMemoryServerFixture _fixture;

        public HttpTunnelMethodClientTests(InMemoryServerFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        {
            var client = _fixture.GetHttpClientWithTunnelOverMethodClient();

            // Perform get
            var uri = new UriBuilder
            {
                Path = "v2/path/test/methodClientTestId"
            }.ToString();
            ...
            Assert.Equal("methodClientTestId", response.Id);
        }
        ...
        public async Task TestPutWithBadPathAsync(ISerializer serializer)
        {
            ...
            var uri = new UriBuilder
            {
                Path = "v2/path/test/methodClientTestId" // No id allowed for put
            }.ToString();
            using var httpRequest = new HttpRequestMessage(HttpMethod.Put, uri);
            serializer.SerializeToRequest(httpRequest, expected);
            var httpResponse = await client.SendAsync(httpRequest);
