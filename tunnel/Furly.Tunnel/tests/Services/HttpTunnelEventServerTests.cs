...
namespace Furly.Tunnel.Services.Tests
{
    ...
    public class HttpTunnelEventServerTests
    {
        [Fact]
        public async Task TestGetWebAsync()
        {
            ...
            using var clientHandler = new HttpTunnelEventClientHandler(eventBridge, eventBridge, _serializer,
            ...
            using var server = new HttpTunnelEventServer(httpClientFactoryServer,
                eventBridge, _serializer, logger.CreateLogger<HttpTunnelEventServer>());
            await server;
            // Act
