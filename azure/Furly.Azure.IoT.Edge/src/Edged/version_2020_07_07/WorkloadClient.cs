...
namespace Microsoft.Azure.Devices.Edge.Util.Edged.Version20200707
{
    using Microsoft.Azure.Devices.Edge.Util.Edged.Version20200707.GeneratedCode;
    ...
    internal sealed class WorkloadClient : WorkloadClientBase
    {
        ...
        public override async Task<string> GetManifestTrustBundleAsync()
        {
            using (var httpClient = GetHttpClient())
            {
                ...
                var result = await ExecuteAsync(() => edgeletHttpClient.ManifestTrustBundleAsync(
                    Version.Name), "ManifestTrustBundleAsync").ConfigureAwait(false);
                return result.Certificate;
            }
        }
        ...
        /// <inheritdoc/>
        public override async Task<string> SignAsync(string keyId, string? algorithm,
            string data)
        {
